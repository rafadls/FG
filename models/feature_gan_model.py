...
class FeatureGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_names = ['D_B_M', 'G_B','G_B_M', 'F_B', 'G', 'idt_B']
        ...
        if self.isTrain:
            ...
            self.model_names = ['G_B', 'D_B_M']
        else:  
            self.model_names = ['G_B']

        self.netG_B = networks.define_G(5, 3, 72, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define discriminators

            self.netD_B_M = networks.define_D(257, 64, "n_layers", 
        ...
        if self.isTrain:
            ...
            self.criterionFeatureRobots = networks.FeatureLoss(0.1*factorAll ,      0.25*factorAll,      0.25*factorAll,      "L1").to(self.device) # L1 o MSE
            ...
            self.jitter = torchvision.transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.05, hue=0.1)

            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_B.parameters()), lr=opt.lr*1.5, betas=(opt.beta1, 0.999))

            #self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_B_M.parameters(), self.netD_B_G.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_B_M.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        a = torch.cat([self.real_B, self.real_B_depth], 1)
        b = torch.cat([a, self.real_B_mask], 1)
        if self.isTrain:
            self.fake_A = self.netG_B(b)
        else:
            self.loss_idt_B = 0
            self.fake_A = self.netG_B(b)
    ...
    def backward_D_B(self):
        ...
        concat_fake_A_M = torch.cat([self.upsample_M(out1[0,:,:,:].unsqueeze(0)), self.upsample_M(out2[0,:,:,:].unsqueeze(0))], 1)
        concat_fake_A_M = torch.cat([self.upsample_M(self.real_B_mask), concat_fake_A_M], 1)
        ...
        concat_real_A_M = torch.cat([ self.upsample_M(out1[1,:,:,:].unsqueeze(0)), self.upsample_M(out2[1,:,:,:].unsqueeze(0))], 1)
        
        concat_real_A_M = torch.cat([self.upsample_M(self.real_A_mask), concat_real_A_M], 1)
...
#--------------------------------Total------------------------------------------------------
    ...
    def optimize_parameters(self,epoch):
        ...
        self.set_requires_grad([self.netD_B_M], True)
        self.set_requires_grad([self.netG_B], False)
        ...
        self.set_requires_grad([self.netD_B_M], False) 
